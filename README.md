# HeadFlight — iPhone GitHub Pages edition 1.2

This package is self-contained. It includes MediaPipe 0.10.21, its WebAssembly files, and the Face Landmarker model. GitHub Actions does not download tracker resources while publishing, and Safari receives every runtime file from the same HTTPS address as the game.

## Update an existing `headflight` repository

Use a computer for the simplest update because the package contains nested `.github`, `scripts`, `vendor`, and `models` folders.
...
- **The old Camera unavailable screen still appears immediately:** verify that the latest GitHub Action is green, close the Safari tab, and reopen the Pages link. Version 1.2 adds `?v=1.2.0` to refresh cached code.
...
- **The message says face-tracking code or model could not load:** confirm that `vendor/vision_bundle.mjs`, all four files under `vendor/wasm/`, and `models/face_landmarker.task` exist in the repository. Then open the GitHub Actions result and confirm the build completed.
...
The ZIP and repository contain the runtime assets directly. The workflow copies them into `dist/`, including:
